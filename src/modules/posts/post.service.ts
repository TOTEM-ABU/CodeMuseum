...
export class PostService {
  ...
  async update(id: string, updatePostDto: UpdatePostDto, userId: string) {
    ...
    if (!post) throw new NotFoundException('Post not found');
    if (post.userId !== userId) throw new BadRequestException('You can only update your own posts');
    ...
    if (updatePostDto.categoryName) {
      ...
      let category = await this.prisma.category.findFirst({ where: { name: categoryName } });
      if (!category) category = await this.prisma.category.create({ data: { name: categoryName } });
  ...
  async remove(id: string, userId: string) {
    ...
    const post = await this.prisma.post.findUnique({ where: { id } });
    if (!post) throw new NotFoundException('Post not found');
    if (post.userId !== userId) throw new BadRequestException('You can only delete your own posts');
